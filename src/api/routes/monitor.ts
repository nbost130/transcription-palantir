...
export async function monitorRoutes(
  ...
  fastify.get('/monitor/queue', {
    ...
    const response: ApiResponse = {
      ...
      data: {
        ...
        throughput: {
          completedLast5Min: completedJobs.filter(j =>
            j.finishedOn && (Date.now() - j.finishedOn) < 300000
          ).length,
          failedLast5Min: failedJobs.filter(j =>
            j.finishedOn && (Date.now() - j.finishedOn) < 300000
          ).length,
        },
      },
  ...
  fastify.get('/monitor/timeline', {
    ...
    const timeline = [
      ...completed.filter(j => j.finishedOn && j.finishedOn > cutoffTime).map(j => ({
      ...
      ...failed.filter(j => j.finishedOn && j.finishedOn > cutoffTime).map(j => ({
        timestamp: j.finishedOn,
        type: 'failed',
        jobId: j.id,
        fileName: j.data.fileName,
